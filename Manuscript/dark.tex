...
PCMs have become extremely popular over recent years; in 2013 alone \citet{harvey1991comparative}'s book on the comparative method in evolutionary biology received 195 citations (Google Scholar, 22nd May 2014), and at the 2014 Evolution meeting in Raleigh NC, we estimate that over XX %need to count these!
...
%Here we explore this problem in more detail.
...
%In this paper we discuss why we think this has happened, and suggest ways to bridge the knowledge gap between methods users and methods developers.  
...
%Why are PCMs increasingly being misused? All evidence points to a knowledge-gap between methods users and methods developers.
...
\begin{enumerate}
\item The assumptions are not mentioned in the literature.
\item The assumptions are mentioned in the literature, but the literature is too dense and technical, or pertinent details are hidden within large sections of text.
\item The assumptions are mentioned in the literature but methods users have gone straight to the implementation of the method and the manual does not mention assumptions


Above we have assumed a knowledge gap, we are not considering cases where The method user is aware of the assumptions and how to test for them, but chooses not to because it means they cannot publish.
