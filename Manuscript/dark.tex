...
% NC: We can cite your paper on Pagel 1994 here if you like - as an example of old methods that are still being investigated.
...
Even with prior knowledge of the key papers and authors to focus on, this resulted in XX pages, XX papers and a book to read. % add numbers
...
This has led to an increasing number of methods-users jumping straight to the implementation of a method, without understanding what the method is doing, what its assumptions might be, or what the results they extract mean in a biological context.
...
The truly sad thing about this state of affairs is that by making science more open and sharing code (something we are all passionate about and strongly support) we seem to have made the situation worse rather than better. Particularly when the flexibility of R should improve the abilit methods \citealp{freckleton2009seven}. %check Freck
...
% NC: Another more %knitr reports in supp mat (e.g. cite that nice paper )
...
Our ability to do science often relies on methods-developers and other highly-skilled technical people, especially as evolutionary biology becomes ever more computationally intensive. 
We cannot afford to lose these people to industry, nor can we pay industry wages, thus we need to make it worthwhile for methods-developers to remain in (or at least interact with) academia.
...
These kinds of papers are arguably more useful to the community than constantly publishing new methods as this encourages methods-users to retrofit questions so they can use the newest shiniest methods available, rather than carefully thinking about the question they are trying to answer. 
Journals also have a part to play in encouraging methods-developers to develop ways of testing the assumptions of their methods and building this into the original publications and implementations.
...
The ``publish or perish'' mentality is not conducive to careful method use or attempts to fully understand a method prior to using it. 
Nor does it encourage methods-developers to spend more time summarizing their work and making their methods more user-friendly. 
If publication quality were considered more important that quantity this would improve career prospects for methods-developers, careful methods-users and also for groups that traditionally publish fewer papers, including women. 
This is particularly important as the people willing to take the time to share their skills are the people we really want as colleagues, not the selfish high achievers with hundreds of papers! 

\section{Conclusion}

We feel that phylogenetic comparative methods have fallen into a bit of a rut in recent years. Their increased popularity and easier implementation have led to numerous sloppy publications.
